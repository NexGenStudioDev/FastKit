...
const envSchema = z.object({
  ...
  SERVER_KEEP_ALIVE: z.string().default('false').transform(val => val === 'true'),
  SERVER_BODY_LIMIT: z.string().default('100mb'),
  SERVER_TRUST_PROXY: z.string().default('false').transform(val => val === 'true'),
  ...
  RATE_LIMIT_WINDOW_MS: z.string().optional().transform(val => val ? Number(val) : undefined),
  RATE_LIMIT_MAX: z.string().optional().transform(val => val ? Number(val) : undefined),
  SECURITY_HELMET: z.string().default('false').transform(val => val === 'true'),
  SECURITY_COMPRESSION: z.string().default('false').transform(val => val === 'true'),
  SECURITY_MORGAN: z.string().default('false').transform(val => val === 'true'),
