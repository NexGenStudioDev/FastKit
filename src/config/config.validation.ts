...
export const emailConfigSchema = z.object({
  service: z.enum(['gmail', 'yahoo', 'outlook', 'sendgrid', 'mailgun', 'ses', 'custom']).optional(),
...
export const envVarsSchema = z.object({
  ...
  PORT: z.string().default('3000').transform(val => parseInt(val)).pipe(portSchema),
  ...
  DATABASE_PORT: z.string().optional().transform(val => val ? parseInt(val) : undefined).pipe(z.number().optional()),
  DATABASE_USERNAME: z.string().optional(),
  DATABASE_PASSWORD: z.string().optional(),
  DATABASE_NAME: z.string().optional(),
  DATABASE_POOL_SIZE: z.string().optional().transform(val => val ? parseInt(val) : undefined).pipe(z.number().optional()),
  DATABASE_CONNECTION_TIMEOUT: z.string().optional().transform(val => val ? parseInt(val) : undefined).pipe(z.number().optional()),
  ...
  JWT_ALGORITHM: z.enum(['HS256', 'HS384', 'HS512', 'RS256', 'RS384', 'RS512', 'ES256', 'ES384', 'ES512']).default('HS256'),
  ...
  CORS_CREDENTIALS: z.string().optional().transform(val => val === 'true'),
  CORS_MAX_AGE: z.string().optional().transform(val => val ? parseInt(val) : undefined).pipe(z.number().optional()),

  // Security Environment Variables (optional)
  RATE_LIMIT_WINDOW_MS: z.string().optional().transform(val => val ? parseInt(val) : undefined).pipe(z.number().optional()),
  RATE_LIMIT_MAX: z.string().optional().transform(val => val ? parseInt(val) : undefined).pipe(z.number().optional()),
  SECURITY_HELMET: z.string().optional().transform(val => val === 'true'),
  SECURITY_COMPRESSION: z.string().optional().transform(val => val === 'true'),
  SECURITY_MORGAN: z.string().optional().transform(val => val === 'true'),
  SECURITY_CSRF: z.string().optional().transform(val => val === 'true'),

  // Email Environment Variables (optional)
  EMAIL_SERVICE: z.enum(['gmail', 'yahoo', 'outlook', 'sendgrid', 'mailgun', 'ses', 'custom']).optional(),
  EMAIL_HOST: z.string().optional(),
  EMAIL_PORT: z.string().optional().transform(val => val ? parseInt(val) : undefined).pipe(z.number().optional()),
  ...
  SERVER_TIMEOUT: z.string().optional().transform(val => val ? parseInt(val) : undefined).pipe(z.number().optional()),
  SERVER_KEEP_ALIVE: z.string().optional().transform(val => val === 'true'),
  SERVER_BODY_LIMIT: z.string().optional(),
  SERVER_TRUST_PROXY: z.string().optional().transform(val => val === 'true'),
}).refine(
  (data) => {
  ...
  {
    message: "For non-MongoDB databases, either DATABASE_URL or both DATABASE_HOST and DATABASE_NAME must be provided",
    path: ['DATABASE_URL']
  }
);
 ...
 */
export function validateEnvData(envData?: Record<string, string | undefined>): {
  isValid: boolean;
  errors: string[];
  ...
  try {
    ...
    if (validatedEnv.CORS_ORIGIN || validatedEnv.CORS_METHODS) {
      config.cors = {
        ...
        methods: validatedEnv.CORS_METHODS ? validatedEnv.CORS_METHODS.split(',') as HTTPMethodType[] : undefined,
        allowedHeaders: validatedEnv.CORS_ALLOWED_HEADERS ? validatedEnv.CORS_ALLOWED_HEADERS.split(',') : undefined,
 ...
 */
export function validateServerConfig(config: unknown): { isValid: boolean; errors: string[]; config?: ServerConfig } {
  try {
 ...
 */
export function validateJWTConfig(config: unknown): { isValid: boolean; errors: string[]; config?: JWTConfig } {
  try {
